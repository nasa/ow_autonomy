...
FaultDependencies::FaultDependencies(const string &file_name,
                                     bool verbose_flag)
{
...
bool FaultDependencies::checkIsOperable(const string &name) const
{
...
bool FaultDependencies::checkIsFaulty(const string &name) const
{
...
vector<string> FaultDependencies::getActiveFaults(const string &name) const
{
...
void FaultDependencies::updateFault(const string &name, int status)
{
  ...
  string parent_subsystem  = m_faults[name].parent_subsystem;


  // Increment local fault flag for parent subsystem and change faulty
  // flag as needed.
  if (status == 1){
...
void FaultDependencies::updateSubsystem(const string &name,
                                        int status,
                                        const string &parent)
{
  ...
  // If subsystem inoperable flag changes, we need to cascade
  // inoperable fault to impacted subsystems
  if (prev_inoperable_state != m_subsystems[name].num_inoperable){
    cascadeFault(name, status);
  }
}

void FaultDependencies::cascadeFault(const string &subsystem_name, int status)
{
  ...
  nodes.push(subsystem_name);
    ...
    while(!nodes.empty()){
      string current_node = nodes.top();
      nodes.pop();

      for (auto i: m_subsystems[current_node].impacts){
        // If it is a procedure we do not need to add it to BFS stack,
        // and can just update the inoperable flag
        ...
        // Otherwise we update subsystem and add it to stack
          ...
          if (visited.insert(i.first).second &&
              m_subsystems.find(i.first) != m_subsystems.end()){
...
void FaultDependencies::DebugPrint() const{
  ...
  for (auto fault: m_faults){
    ROS_INFO_STREAM(std::left << std::setw(40) << fault.second.name
                    << std::setw(20) << fault.second.faulty );
  }
  ...
  ROS_INFO_STREAM(std::setw(25) << std::left << "NAME" << std::setw(17)
                  << "TYPE" << std::setw(17)
                  << "NON-LOCAL FAULT" << std::setw(17) << "LOCAL FAULT"
                  << std::setw(17) << "FAULTY" << "INOPERABLE" );
  ROS_INFO_STREAM("------------------------------------------------------------"
            << "-----------------------------------------" 
            );
  for (auto subsystem: m_subsystems){
    ROS_INFO_STREAM(std::left << std::setw(25) << subsystem.second.name
                    << std::setw(17) << "Subsystem"
                    << std::setw(17) << subsystem.second.num_non_local_faults
                    << std::setw(17) << subsystem.second.num_local_faults
                    << std::setw(17) << subsystem.second.faulty
                    << subsystem.second.num_inoperable );
  }
  for (auto proc: m_procedures){
    ROS_INFO_STREAM(std::left << std::setw(25) << proc.second.name
                    << std::setw(17) << "Procedure" << std::setw(17)
                    << "-" << std::setw(17) << "-" << std::setw(17)
                    << "-" << proc.second.num_inoperable );
  }
}

bool FaultDependencies::parseXML(const string &file)
{
  ...
  for (pugi::xml_node subsystem = subsystems.child("Subsystem");
       subsystem;
       subsystem = subsystem.next_sibling("Subsystem")) {
    // get name and severity thresholds
    string subsystem_name = subsystem.attribute("Name").as_string();
    ...
    // get all affected subsystems and add them to their respective
    // parent subsystem
    for (pugi::xml_node target = subsystem.child("Impacts"); target;
         target = target.next_sibling("Impacts")){
      if (target.empty()){
        ROS_ERROR_STREAM("Warning: failed to read an impacts target "
                         << target << "\n");
        return false;
      }
      string target_name = target.attribute("Name").as_string();
      string target_type = target.attribute("Type").as_string();

      m_subsystems[subsystem_name].impacts.push_back(std::make_pair(target_name,
                                                                    target_type));

      if (target_type == "Procedure" &&
          m_procedures.find(target_name) == m_procedures.end()){
        m_procedures[target_name] = Procedure();
        m_procedures[target_name].name = target_name;
      }
    }

    // Read all faults and add them to m_faults as well as their
    // respective subsystems
    for (pugi::xml_node fault = faults.child("Fault");
         fault;
         fault = fault.next_sibling("Fault")){
      string fault_name = fault.attribute("Name").as_string();
      if (fault_name.empty()){
        ROS_ERROR_STREAM("Warning: failed to read a fault in subsystem "
                         << subsystem_name << "\n");
        return false;
      }
      ...
      for (pugi::xml_node impact = fault.child("Impacts");
           impact;
           impact = impact.next_sibling("Impacts")){
        string impact_name = impact.attribute("Name").as_string();
        string impact_type = impact.attribute("Type").as_string();
        m_faults[fault_name].impacts.push_back(std::make_pair(impact_name,
                                                              impact_type));
        if (impact_type == "Procedure" &&
            m_procedures.find(impact_name) == m_procedures.end()){
